...
async function generateUpdatedSummary(oldSummary, trimmedLines) {
  const serializedTrimmedText = trimmedLines
    .map(
      (msg) =>
        `${msg.role === "user" ? "Player Action" : "DM Response"}: ${msg.parts[0].text}`,
    )
    .join("\n");
...
`;
  ...
  try {
    ...
    console.log(
      `[SUMMARY ARCHIVE]: Current log footprint is ${tokenCheck.totalTokens} tokens.`,
    );
    ...
    if (tokenCheck.totalTokens > GLOBAL_SUMMARY_TOKEN_LIMIT) {
      console.warn(
        `[TOKEN CAPACITY REACHED]: Summary reached ${tokenCheck.totalTokens} tokens. Executing archival deep-compaction...`,
      );
...
`;
    ...
    console.error(
      "Summary execution or token verification cycle failed, reverting state:",
      err,
    );
    return oldSummary;
  }
...
app.post("/command", async (req, res) => {
  try {
    ...
    // 🎒 COMMAND ID 4: UPDATE INVENTORY
      ...
      await docRef.set(
        { playerCards: { [userRefId]: { inventory: currentInventory } } },
        { merge: true },
      );
    ...
    // 🔮 COMMAND ID 5: LEARN NEW SPELL
      ...
      await docRef.set(
        { playerCards: { [userRefId]: { learnedSpells: currentSpells } } },
        { merge: true },
      );
    ...
    // 📖 COMMAND ID 7: SHOW SPELLBOOK
      ...
      spellbookOutput +=
        spells.length === 0
          ? `\n_Your memory holds no custom incantations._`
          : spells.map((spell) => `  ✨ ${spell}`).join("\n");
    ...
    return res.json(
      formatChatResponse(
        "*The DM drops the dice:* Something went wrong processing your check.",
        null,
      ),
    );
  }
...
app.post("/chat-bot", async (req, res) => {
  try {
    ...
    const docRef = getDocRef(sessionId);
    let { history, playerCards, campaign, currentTurn, campaignSummary } =
    ...
    const currentTurnContext = {
      ...
      parts: [
        {
          text: `[TURN CONTEXT]: Game Turn Count: ${currentTurn} | Active user ID: "${userRefId}". Evaluate choices matching this profile.`,
        },
      ],
    };
    ...
    if (history.length >= MAX_HISTORY_LENGTH) {
      console.log(
        `[COMPACTION TRIGGERED]: Chat thread reached ${history.length} lines. Condensing back to ${SHRINK_HISTORY_LENGTH} rows...`,
      );
      ...
      campaignSummary = await generateUpdatedSummary(
        campaignSummary,
        linesToTrim,
      );
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(
    `D&D Bot backend listening on port ${PORT} bound successfully to 0.0.0.0`,
  );
});
